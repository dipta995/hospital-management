...
<div class="container">
    ...
    <div class="card-body">
        ...
        <h4 class="card-title" style="font-size: 20px; font-weight: bold; text-align: center; margin-top:10px;">{{ $pageHeader['title'] }}'s Salary Report</h4>
        ...
        <div class="table-responsive">
            <table class="table table-striped mt-3">
                ...
                @php
                    $sal=0;
                @endphp
                @foreach($groupedSalaries as $employeeName => $salaries)
                    @php
                    $sal+=$salaries->sum('salary');
                    @endphp
                    <tr>
                        <td colspan="4" class="bg-primary text-white fw-bold">{{ $employeeName }}</td>

                        <td>{{ $salaries->sum('salary') }}</td>
                    </tr>
...
{{--                            <td>{{ $item->year }}</td>--}}
                            ...
                            <td>{{ $item->note }} </td>
                            <td>{{ $item->refer_by ?? 'N/A' }}</td>
                            <td>{{ $item->created_at->format('d M Y') }}</td>
                            <td class="badge bg-success text-white">{{ $item->payment_type }}</td>
            ...
            <div class="d-flex justify-content-end">
                <h4 style="text-align: right; margin: 20px 20px;">Total : {{ $sal }} Taka</h4>
                {{--                                    {!! $datas->links() !!}--}}
            </div>
        </div>
    </div>
