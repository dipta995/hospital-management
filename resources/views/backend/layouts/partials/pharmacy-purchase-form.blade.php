...
<form method="post" action="javascript:void(0);" id="purchase-form">
    ...
    <div class="pharm-pos-grid pharm-purchase-grid">
        ...
        <div class="pharm-purchase-main">
            ...
            {{-- Add product draft row --}}
            <div class="pharm-pos-panel mb-3">
                <div class="pharm-pos-panel-head"><i class="fas fa-plus-circle"></i> Add Product to Purchase</div>
                <div class="pharm-pos-panel-body">
                    <div class="row g-2 align-items-end">
                        <div class="col-lg-4 col-md-6">
                            <label class="form-label">Product</label>
                            <select id="draft_product" class="form-select">
                                <option value="">Search & select product...</option>
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}"
                                        data-name="{{ e($product->name) }}"
                                        data-generic="{{ e($product->generic_name ?? '') }}"
                                        data-price="{{ $product->purchase_price }}"
                                        data-stock="{{ $product->current_stock ?? 0 }}">
                                        {{ $product->name }}@if($product->generic_name) — {{ $product->generic_name }}@endif
                                    </option>
                                @endforeach
                            </select>
                            <div class="small text-muted mt-1">Current stock: <strong id="draft_stock">—</strong></div>
                        </div>
            ...
            <div class="pharm-pos-panel">
                ...
                <div class="table-responsive">
                    <table class="table pharm-cart-table pharm-purchase-table mb-0">
                        ...
                        @foreach($purchaseItems as $line)
                            @include('backend.layouts.partials.pharmacy-purchase-item-row', [
                                'line' => $line,
                ...
                <div id="purchase-empty-state" class="pharm-purchase-empty @if($purchaseItems->isNotEmpty()) d-none @endif">
                    ...
                    <span class="small text-muted">Select a product above and click <strong>Add Line</strong></span>
                </div>
            </div>
        </div>
            ...
            <div class="pharm-pos-panel-body">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" step="0.01" min="0" id="paid_amount" class="form-control form-control-lg"
                        value="{{ $isEdit ? $purchase->paid_amount : 0 }}">
                </div>
                <div class="mb-4">
                ...
                <div class="d-grid gap-2">
                    ...
                    <a href="{{ route('admin.pharmacy_purchases.index') }}" class="btn btn-outline-secondary">Cancel</a>
                </div>
            </div>
        </div>
    </div>
...
<tr class="item-row">
    <td>
        <div class="pharm-cart-product">
            ...
            <div class="small text-muted product-generic d-none"></div>
        </div>
        <input type="hidden" class="pharmacy_product_id" value="">
    </td>
