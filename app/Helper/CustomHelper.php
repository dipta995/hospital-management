...
use App\Models\SettingRequisitionApproval;
use App\Models\User;
use Carbon\Carbon;
...
//  const NidRegex = "/(?:\d{17}|\d{13}|\d{10})/";
  ...
  public static function deleteFile($image = null) {
   ...
   public static function imageUpload($file, $file_path = null)
    {
   ...
   public static function removeOldImage($file)
    {
        unset($file);
    }
   public static function dateFormetChange($dateString)
    {
        if ($dateString === null) {
            $formattedDate = date('Y-m-d'); // Use current date if $dateString is null
        } else {
   ...
   * @return bool
   */
  public static function settingRequisitionApprovalUserCheck() {
    if (auth()->user() && $app = SettingRequisitionApproval::where('user_id',auth()->id())->first()){
      return $app->level;
    }
    return false;
  }

    public static function startIndexDynamic($datas)
    {
        $perPage = $datas->perPage(); // Number of items per page
        $currentPage = $datas->currentPage(); // Current page number
